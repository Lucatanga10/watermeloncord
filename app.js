...
// "Hash" molto semplice (frontend only — non sicuro, ma evita di salvare la password in chiaro)
function hashPwd(pwd) {
  let h = 5381;
  for (let i = 0; i < pwd.length; i++) h = ((h << 5) + h + pwd.charCodeAt(i)) | 0;
  return "p_" + (h >>> 0).toString(36) + "_" + pwd.length;
}
...
// Riallinea state con localStorage (per cogliere account creati su altre tab)
function reloadStateFromStorage() {
  try {
    const raw = localStorage.getItem(STORE_KEY);
    if (raw) {
      const myCurrent = state.currentUserId;
      const parsed = JSON.parse(raw);
      state = parsed;
      state.currentUserId = myCurrent;
    }
  } catch (e) {}
}
...
$("loginBtn").addEventListener("click", () => {
  ...
  const target = Object.values(state.users).find(u => u.name && u.name.toLowerCase() === name.toLowerCase());
  if (!target) { showLoginError("Utente non trovato."); return; }
  // Account vecchi (senza password): permetti il primo login e imposta la pwd
  if (!target.passwordHash) {
    target.passwordHash = hashPwd(pwd);
  } else if (target.passwordHash !== hashPwd(pwd)) {
...
$("registerBtn").addEventListener("click", () => {
  ...
  if (!pwd || pwd.length < 4) { showRegisterError("La password deve avere almeno 4 caratteri."); return; }
...
function startHeartbeat() {
  ...
  if (me) { me.lastSeen = now(); save(); }
  heartbeatInterval = setInterval(() => {
    const me2 = state.users[state.currentUserId];
    if (me2) { me2.lastSeen = now(); save(); }
...
$("profileEditSave").addEventListener("click", () => {
  ...
  if (profileEditBannerData) u.banner = profileEditBannerData;
  save();
